...
export class ImageService {
  ...
  saveImage(image: ImageNoId): Observable<ImageNoId> {
    return this.http.post<ImageNoId>('http://localhost:8080/image', image)
  }
  ...
  findByAccount_IdAAndStatusImg1(accountId:number):Observable<Image1[]>{
    return this.http.get<Image1[]>(`http://localhost:8080/image/a/1/${accountId}`)
  }

  findByAccount_IdAAndStatusImg2(accountId:number):Observable<Image1[]>{
    return this.http.get<Image1[]>(`http://localhost:8080/image/a/2/${accountId}`)
  }
}
